...
<div class="container mx-auto py-10">
  <table class="min-w-full bg-white shadow-md rounded-lg overflow-hidden">
   ...
   <tr class="border-b border-gray-200 hover:bg-gray-50">
        ...
        <td class="px-4 py-2 text-lg font-bold text-red-800">{site.block_reports}</td>
        <td class="px-4 py-2 text-lg font-bold text-green-600">{site.unblock_reports}</td>
        <td class="px-4 py-2 ">{new Date(site.last_reported_at).toLocaleString()}</td>
        <td class="px-4 py-2 ">{site.categories.join(" , ")}</td>
        <td class="px-4 py-2 "><a href={"/sites/"+site.id}><button class="btn-primary btn">View</button></a></td>
      </tr>
    {/each}
    </tbody>
  </table>
